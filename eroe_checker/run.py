...
def check_eroi(driver):
    with open("profile.json") as fp:
        data = json.load(fp)
        for p in data:
            try:
                driver.get(p["p"])
                time.sleep(3)
                foto = driver.find_element(By.XPATH,"//*[@id='content']/img")
                urllib.request.urlretrieve(foto.get_attribute('src'), "./"+p["p"]+".png")
...
def raidStats(driver):
    driver.get("https://ts31.x3.europe.travian.com/statistics/player?idSub=3")
    time.sleep(2)
...
def npc_grano_negativo(driver,village,aid):
    ...
    if risorse["q_grano"] >= 95 and risorse["q_media"] < 70:
        if "dorf2" not in driver.current_url:
            driver.find_element(By.XPATH,"//a[@accesskey='2']").click()
        driver.get(f"https://ts31.x3.europe.travian.com/dorf2.php?newdid={village}")
        time.sleep(2)
...
def get_chromedriver():
    ...
    chrome_options.add_argument("--user-data-dir=/home/edoardo/.config/google-chromeTrav2")
...
def overview(driver):
    roma_troops(driver)
    #dracarys_troops(driver)
    amsterdam_troops(driver)
    for x in range(0,1):
        driver.get(f"https://ts31.x3.europe.travian.com/dorf2.php?newdid=20900")
...
def roma_troops(driver):
    driver.get(f"https://ts31.x3.europe.travian.com/dorf2.php?newdid=19087")
    time.sleep(1)
...
def amsterdam_troops(driver):
    driver.get(f"https://ts31.x3.europe.travian.com/dorf2.php?newdid=27117")
    time.sleep(1)
    parse_risorse(driver)
...
def dracarys_troops(driver):
    ...
    if "statistics" not in driver.current_url:
        ...
        time.sleep(1)
    time.sleep(3)
    dracarys = driver.find_element(By.XPATH,"//*[@id='overview']/tbody/tr[2]")
    ...
    if mazze_time < tk_time:

        if mazze_time < CODA_MIN:
            da_incodare = int((CODA_MIN+30 - mazze_time)//MAZZE_SEC)
            print(da_incodare)
            truppav2(driver,"mazze",da_incodare,"20900",30)
        ...
        if tk_time < CODA_MIN:
            da_incodare = int((CODA_MIN+30 - tk_time)//TK_SEC)
            print(da_incodare)
            truppav2(driver,"tk",da_incodare,"20900",32)

    if cata_time < CODA_MIN:
        da_incodare = int((CODA_MIN+30 - cata_time)//CATA_SEC)
        print(da_incodare)
        truppav2(driver,"cata",da_incodare,"20900",35)

    # if arieti_time < CODA_MIN:
    #     da_incodare = int((CODA_MIN+20 - arieti_time)//ARIETI_SEC)
    #     print(da_incodare)
    #     truppav2(driver,"arieti",da_incodare,"20900",35)
...
def attack_oasis(driver):
    ...
    if "Cavalieri Teutonici" in troops:
        ...
        for line in oasis:
            try:
                ...
                driver.get(f"https://ts31.x3.europe.travian.com/position_details.php?x={line['x']}&y={line['y']}")
                time.sleep(2)
    ...
    driver.get("https://ts31.x3.europe.travian.com/dorf2.php")
...
def login(driver):
    ...
    driver.get("https://ts31.x3.europe.travian.com")
    
    #wait(driver,"//form[@name='login']",20,False)
    driver.find_element(By.XPATH,"//input[@name='name']").send_keys("Targaryen")
    time.sleep(1)
    driver.find_element(By.XPATH,"//input[@name='password']").send_keys("Travian.Targaryen1")
    time.sleep(2)
...
def farm(driver,village):
    ...
    if village == "20900":
        parse_troops(driver)
        if "Combattenti" in troops and troops["Combattenti"] >= 1200:
            send_farmv2(driver,troops["Combattenti"],village,(4,8))
            driver.find_element(By.XPATH,"//a[@accesskey='2']").click()
        
        elif "Cavalieri Teutonici" in troops and troops["Cavalieri Teutonici"] >= 1000:
            print("OK TK")
            send_farmv2_tk(driver,troops["Cavalieri Teutonici"],village,(0,4))
            driver.find_element(By.XPATH,"//a[@accesskey='2']").click()
    ...
    if village == "27117":
        ...
        if "Combattenti" in troops and troops["Combattenti"] >= 1000:
            send_farmv2_am(driver,troops["Combattenti"],village,(4,7))
            driver.find_element(By.XPATH,"//a[@accesskey='2']").click()

        elif "Cavalieri Teutonici" in troops and troops["Cavalieri Teutonici"] >= 950:
            print("OK TK")
            send_farmv2_am_tk(driver,troops["Cavalieri Teutonici"],village,(0,4))
            driver.find_element(By.XPATH,"//a[@accesskey='2']").click()

    if village == "19087":
        parse_troops(driver)
        # if "Combattenti" in troops and troops["Combattenti"] >= 850:
        #     send_farmv2_am(driver,troops["Combattenti"],village,(3,7))
        #     driver.find_element(By.XPATH,"//a[@accesskey='2']").click()

        if "Cavalieri Teutonici" in troops and troops["Cavalieri Teutonici"] >= 150:
            print("OK TK")
            send_farmv2_am_tk(driver,troops["Cavalieri Teutonici"],village,(0,1))
            driver.find_element(By.XPATH,"//a[@accesskey='2']").click()
    ...
    # if village == "19300":
    #     if "Combattenti" in troops and troops["Combattenti"] >= 15:
    #         send_farm(driver,troops["Combattenti"]-15,village)
    #         driver.find_element(By.XPATH,"//a[@accesskey='2']").click()
...
def truppav2(driver,unita,n,village,aid):
    ...
    if (unita == "mazze" and risorse["legno"] > 95 and risorse["argilla"] > 75 and risorse["ferro"] > 40 and risorse["grano"] > 40) or (unita == "mazzeg" and risorse["legno"] > 285 and risorse["argilla"] > 225 and risorse["ferro"] > 120 and risorse["grano"] > 120):
            ...
            try:
                driver.get("https://ts31.x3.europe.travian.com/hero")
                time.sleep(1)
    ...
    if (unita == "tk" and risorse["legno"] > 450 and risorse["argilla"] > 515 and risorse["ferro"] > 480 and risorse["grano"] > 80) or (unita == "tkg" and risorse["legno"] > 1350 and risorse["argilla"] > 1545 and risorse["ferro"] > 1440 and risorse["grano"] > 240):
            ...
            try:
                n_troops_available = int(driver.find_element(By.XPATH,"//*[@id='nonFavouriteTroops']/div[6]/div/div[2]/div[4]/a").text)
                n_troops = int(max(1,min(n,n_troops_available)*0.45))
                if n_troops == 0:
                    return
            except Exception as e:
...
def coloni(driver):
    if risorse["totale"] >= 20001:
        driver.find_element(By.XPATH,"//a[@accesskey='2']").click()
        time.sleep(2)
        driver.get("https://ts31.x3.europe.travian.com/build.php?id=29&gid=25")
        time.sleep(3)
        driver.find_element(By.XPATH,"//*[@value='Scambio risorse']").click()
        #and not contains(@class, 'disabled')
        time.sleep(2)
